...
int particao(int *nums, int esquerda, int direita)
{
	int pivo = nums[direita];
	int i = esquerda - 1;
...
void quickSort(int *nums, int esquerda, int direita)
{
	if(esquerda < direita)
	{
		int pivo = particao(nums,esquerda,direita);

		quickSort(nums,esquerda,pivo-1);
		quickSort(nums,pivo+1,direita);
	}
}
...
int fourSumCount( int *nums1, int nums1Size, int *nums2, int nums2Size,
    ...
    int *s1 = malloc(tamanho * sizeof(int));
    int *s2 = malloc(tamanho * sizeof(int));

    
    GerarSomasDePares(s1, nums1, nums2, n);
    GerarSomasDePares(s2, nums3, nums4, n);

    quickSort(s1, 0, tamanho - 1);
    quickSort(s2, 0, tamanho - 1);
